<template>
  <div class="flex flex-col items-center justify-center">
    <h3 class="theme-title text-2xl font-semibold mb-4">{{ $t('menu_coupon') }}</h3>
    <UTabs v-model="activeTab" :items="tabList" class="w-full">
      <template #icon="{ item, selected }">
        <UIcon
          :name="item.icon"
          class="w-4 h-4 flex-shrink-0 mr-2"
          :class="[selected && 'text-amber-500 dark:text-amber-300']"
        />
      </template>
    </UTabs>
    <CouponToProduct v-if="activeTab === 0" v-on:active-tab="onActiveTab" />
    <MultipleTransaction v-if="activeTab === 1" :channel="TransactionChannel.COUPON" />
  </div>
</template>

<script lang="ts" setup>
import { TransactionChannel } from '~/models/transactions.model';

const { t } = useI18n()

const activeTab = ref<number>(0)
const tabList = [
  {
    name: 'coupon',
    label: t('coupon_to_product'),
    icon: 'i-heroicons-receipt-percent',
  },
  {
    name: 'history',
    label: t('history_short'),
    icon: 'i-heroicons-clock',
  },
]

const onActiveTab = (tab: number) => (activeTab.value = tab)
</script>
